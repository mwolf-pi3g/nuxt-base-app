<template>
  <v-container>
    <v-row>
      <v-col cols="12">
        <p class="text-body-1 mb-6">Manage how you receive your reports.</p>
      </v-col>
    </v-row>
...
const generateZodRules = (token: any) => {
  ...
  if (token.type === 'bool') {
    ...
    if (typeof token.min === 'number') {
      numSchema = numSchema.min(token.min, `Must be at least ${token.min}`);
    }
    if (typeof token.max === 'number') {
      numSchema = numSchema.max(token.max, `Must be at most ${token.max}`);
    }
    schema = numSchema;
  } else {
    let strSchema = z.string().trim().min(1, 'Must be at least 1 character').max(64, 'Must be at most 64 characters');
    if (token.regex && Array.isArray(token.regex)) {
      const pattern = token.regex[0];
      const flags = token.regex[1] || '';
      strSchema = strSchema.regex(new RegExp(pattern, flags), 'Invalid format');
    }
    schema = strSchema;
  }
